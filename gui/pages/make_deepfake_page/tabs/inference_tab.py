...
from core.worker import Worker, InferDFModelWorker
from enums import CONNECTION, DEVICE, FACE_DETECTION_ALGORITHM, JOB_TYPE, SIGNAL_OWNER
...
class InferenceTab(BaseWidget):
    ...
    stop_infer_df_model_sig = qtc.pyqtSignal()
    ...
    def _init_ui(self):
        ...
        start_btn.clicked.connect(self._infer_df_model)
    ...
    @qtc.pyqtSlot()
    def _infer_df_model(self) -> None:
        # but = qwt.QMessageBox.StandardButton.Ok | \
        #     qwt.QMessageBox.StandardButton.Cancel
        # ret = qwt.QMessageBox.question(
        #     self,
        #     'question title',
        #     'question',
        #     but,
        # )
        data_path = r'C:\Users\tonkec\Documents\deepfake\data\trump_cut.mp4'
        device = DEVICE.CUDA
        thread = qtc.QThread()
        worker = InferDFModelWorker(
            data_path,
            device,
            self.signals[SIGNAL_OWNER.MESSAGE_WORKER],
        )
        self.stop_infer_df_model_sig.connect(
            lambda: worker.conn_q.put(CONNECTION.STOP)
        )
        worker.moveToThread(thread)
        self._threads[JOB_TYPE.INFER_DF_MODEL] = (thread, worker)
        worker.finished.connect(self._on_infer_df_model_worker_finished)
        thread.started.connect(worker.run)
        thread.start()

    @qtc.pyqtSlot()
    def _on_infer_df_model_worker_finished(self) -> None:
        val = self._threads.get(JOB_TYPE.INFER_DF_MODEL, None)
        if val is not None:
            thread, _ = val
            thread.quit()
            thread.wait()
            self._threads.pop(JOB_TYPE.INFER_DF_MODEL, None)
