...
from core.model.pg_modules.blocks import FeatureFusionBlock


def _make_scratch_ccm(scratch, in_channels, cout, expand=False):
    # shapes
    out_channels = [cout, cout * 2, cout * 4,
                    cout * 8] if expand else [cout] * 4
...
def _make_scratch_csm(scratch, in_channels, cout, expand):
    ...
    scratch.CHANNELS = [cout, cout, cout * 2,
                        cout * 4] if expand else [cout] * 4
...
def _make_efficientnet(model):
    ...
    pretrained.layer0 = nn.Sequential(
        model.conv_stem, model.bn1, nn.ReLU6(True),
        *model.blocks[0: 2])
...
def _make_projector(im_res, cout, proj_type, expand=False):
    ...
    # determine resolution of feature maps, this is later used to calculate
    # the number of down blocks in the discriminators. Interestingly, the best
    # results are achieved by fixing this to 256, ie., we use the same number
    # of down blocks per discriminator independent of the dataset resolution
    im_res = 256
    pretrained.RESOLUTIONS = [
        im_res // 4,
        im_res // 8,
        im_res // 16,
        im_res // 32,
    ...
    scratch = _make_scratch_ccm(
        scratch,
        in_channels=pretrained.CHANNELS,
        cout=cout,
        expand=expand,
    )
...
class F_RandomProj(nn.Module):
    ...
    def forward(self, x, get_features=False):
        ...
        # start enumerating at the lowest layer (this is where we put the first
        # discriminator)
        backbone_features = {
            '0': out0,
            '1': out1,
        ...
        out2_scale_mixed = self.scratch.layer2_csm(
            out3_scale_mixed, out2_channel_mixed)
        out1_scale_mixed = self.scratch.layer1_csm(
            out2_scale_mixed, out1_channel_mixed)
        out0_scale_mixed = self.scratch.layer0_csm(
            out1_scale_mixed, out0_channel_mixed)
