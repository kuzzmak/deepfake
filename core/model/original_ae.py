...
class OriginalAE(DeepfakeAEModel):
    ...
    def encoder(self, x: torch.Tensor) -> torch.Tensor:
        for channels in self.encoder_channels:
            ...
            x = conv(x)
        x = self.flatten(x)                     # [1, 719872]
        x = self.linear_1024(x)                 # [1, 1024]
        x = self.linear_16384(x)                # [1, 16384]
        x = torch.reshape(x, (-1, 1024, 4, 4))  # [1, 1024, 4, 4]
        x = self.upscale_512_e(x)               # [1, 512, 8, 8]
        return x

    def decoder(self, x: torch.Tensor) -> torch.Tensor:
        x1 = self.upscale_512_d_1(x)            # [1, 512, 16, 16]
        x1 = self.res_block_512_d_1(x1)         # [1, 512, 16, 16]
        x1 = self.upscale_256_d_1(x1)           # [1, 256, 32, 32]
        x1 = self.res_block_256_d_1(x1)         # [1, 256, 32, 32]
        x1 = self.upscale_128_d_1(x1)           # [1, 128, 64, 64]
        x1 = self.res_block_128_d_1(x1)         # [1, 128, 64, 64]
        x1 = self.upscale_64_d_1(x1)            # [1, 64, 128, 128]
        x1 = self.conv_d_1(x1)                  # [1, 3, 128, 128]

        x2 = self.upscale_512_d_2(x)            # [1, 512, 16, 16]
        x2 = self.upscale_256_d_2(x2)           # [1, 256, 32, 32]
        x2 = self.upscale_128_d_2(x2)           # [1, 128, 64, 64]
        x2 = self.upscale_64_d_2(x2)            # [1, 64, 128, 128]
        x2 = self.conv_d_2(x2)                  # [1, 1, 128, 128]

        return x1, x2

    def _calculate_linar_input_size(self, last_conv_filters: int):
