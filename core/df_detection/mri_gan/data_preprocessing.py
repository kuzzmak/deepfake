...
from core.df_detection.mri_gan.deep_fake_detect.features import (
    generate_DFDC_MRIs,
...
from core.df_detection.mri_gan.data_utils.face_mri import generate_MRI_dataset
from core.df_detection.mri_gan.data_utils.utils import \
    get_dfdc_training_video_filepaths
from core.df_detection.mri_gan.utils import ConfigParser, print_line
import core.df_detection.mri_gan.data_utils.augmentation as augmentation
import core.df_detection.mri_gan.data_utils.distractions as distractions


def test_data_augmentation(input_file, output_folder):
    """
    Test all kinds of data augmentation. Applies supported augmentation to
    input_file and save individual output videos in output_folder
    :param input_file:
    :param output_folder:
    :return:
    """
    ...
    augmentation_param = [
        augmentation.get_augmentation_setting_by_type(m)
        for m in augmentation_methods
    ]
    ...
    with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
        ...
        for aug in augmentation_plan:
            ...
            jobs.append(
                pool.apply_async(
                    augmentation.apply_augmentation_to_videofile,
                    (input_file, outfile,),
                    dict(
                        augmentation=aug_func, augmentation_param=aug_param,
...
def test_data_distraction(input_file, output_folder):
    """
    Test all kinds of data distraction. Applies supported distraction to
    input_file and save individual output videos in output_folder
    :param input_file:
    :param output_folder:
    :return:
    ...
    distraction_params = [
        distractions.get_distractor_setting_by_type(m)
        for m in distraction_methods
    ]
    ...
    with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
        ...
        for distract in distraction_plan:
            ...
            jobs.append(
                pool.apply_async(
                    distractions.apply_distraction_to_videofile,
                    (input_file, outfile),
                    dict(
                        distraction=distract_func,
                        distraction_param=distract_param,
                        save_intermdt_files=True,
                    ),
                )
            )
...
def generate_data_augmentation_plan(root_dir=None, plan_pkl_file=None):
    ...
    for i in dist_noise_aug_samples:
        ...
        plan.append({'augmentation': ('noise', noise_param)})
        plan.append(
            {'augmentation': augmentation.get_random_augmentation(
                avoid_noise=True)})
        entry = i, plan
...
def execute_data_augmentation_plan(
    data_augmentation_plan_filename: str,
    metadata_folder: str,
):
    max_num_plans = 4
    ...
    # i wish to apply plan in sequence for each file, but if the file has
    # multiple plans later plans may get started before earlier one finishes.
    # to workaround this, execute plan[0] for each file, then plan[1] for each
    # file
    with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
        jobs = []
        results = []
        for plan_id in range(max_num_plans):
            ...
            for filename, plan in tqdm(data_augmentation_plan, desc=desc):
                if len(plan) > plan_id:
                    p = plan[plan_id]
                    if check_plan_already_executed(
                            filename, p, metadata_folder):
                        continue
                    if 'augmentation' in p.keys():
                        ...
                        jobs.append(
                            pool.apply_async(
                                ...
                                (filename, filename,),
                                dict(
                                    augmentation=distr[0],
                                    augmentation_param=distr[1])))
                        ...
                        jobs.append(
                            pool.apply_async(
                                ...
                                dict(
                                    distraction=distr[0],
                                    distraction_param=distr[1])))

        for job in tqdm(jobs, desc="Executing data augmentation plan"):
            r = job.get()
            try:
                df = pd.Series(r).reset_index().set_index('index')
                b_n = os.path.basename(r['input_file'])
                date = str(datetime.now().strftime("%d-%b-%Y_%H_%M_%S"))
                rfilename = b_n + '_' + date + '.csv'
                df.to_csv(
                    os.path.join(metadata_folder, rfilename), header=False
                )
...
def main():
    if args.apply_aug_to_sample:
        ...
        print(
            'Applying augmentation and distraction to sample ' +
            f'file {sample_test_file}'
        )
        ...
        shutil.copy(
            ...
            os.path.join(
                aug_output_folder,
                os.path.basename(sample_test_file)))
    ...
    if args.gen_aug_plan:
        ...
        generate_data_augmentation_plan(
    ...
    if args.apply_aug_to_all:
        ...
        execute_data_augmentation_plan(
            ConfigParser.getInstance().get_data_aug_plan_pkl_filename(),
    ...
    if args.gen_mri_dataset:
        ...
        # try:
        #     dir_path = r'C:\Users\tonkec\Desktop\deepfake_data\dfdc\mri'
        #     os.remove(
        #         r'C:\Users\tonkec\\Desktop\\mri_gan\assets\\dfdc_mri_metadata.csv')
        #     folders = os.listdir(dir_path)
        #     folders = [os.path.join(dir_path, f) for f in folders]
        #     [os.rmdir(f) for f in folders]
        # except Exception:
        #     pass
