...
class DeepFakeDetectModel(nn.Module):
    """
    This is simple model which takes in each frame of video independently and
    classified them. Later the entire video is classified based upon
    heuristics, which is not done by this model. For the frame passed,
    features are extracted, using given encoder. Then applies
    AdaptiveAvgPool2d, flattens the features and passes to classifier.

    cnn_encoder:
      default: ['tf_efficientnet_b0_ns', 'tf_efficientnet_b7_ns'] # choose
        either one
    training:
      train_size: 1
      valid_size: 1
      ...
      params:
        ...
        train_transform: ['simple', 'complex'] # choose either of the data
          augmentation
