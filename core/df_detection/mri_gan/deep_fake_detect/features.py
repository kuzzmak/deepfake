        ...
        try:
            with multiprocessing.Pool(processes=processes) as pool:
                ...
                for vidx in tqdm(range(video_ids_len), desc="Scheduling jobs"):
                    jobs.append(
                        pool.apply_async(
                            predict_mri_using_MRI_GAN,
                            (
                                crops_path,
                                mri_path,
                                os.path.basename(video_ids_path[vidx]),
                                imsize,
                            ),
                        )
                    )
            ...
            if retry_count <= 0:
                ...
                print(
                    'Retry with lower number of processes. ' +
                    f'processes = {processes}'
                )
                retry_count = MAX_RETRIES
            else:
                print(
                    f'Retry with same number of processes. ' +
