...
from core.df_detection.mri_gan.deep_fake_detect.utils import (
    ...
    get_predictions,
    get_probability,
    global_minibatch_number,
    save_all_model_results,
)
from core.df_detection.mri_gan.utils import ConfigParser, print_green
...
def train_model(log_dir=None, train_resume_checkpoint=None):
    ...
    tqdm_train_descr = tqdm_train_descr_format.format(0, float('inf'), float(
        'inf'), float('inf'), 0, float('inf'), float('inf'), float('inf'))
    ...
    for e in tqdm_train_obj:
        ...
        tqdm_descr = tqdm_train_descr_format.format(
            t_epoch_accuracy,
            t_epoch_loss,
            t_epoch_fake_loss,
            t_epoch_real_loss,
...
def valid_epoch(
        ...
        for batch_id, samples in enumerate(valid_data_iter):
            ...
            for i in range(batch_size):
                all_filenames.append(
                    str(samples['video_id'][i].item()) + '__' +
                    str(samples['frame'][i]))
    ...
    return (
        mean_epoch_acc,
        mean_epoch_loss,
        mean_epoch_fake_loss,
        mean_epoch_real_loss,
        all_predicted_labels,
        all_ground_truth_labels,
        all_filenames,
        probabilities,
