...
def print_batch_item(index, item, all_frames=False, simple=True):
    if simple:
        ...
        if item is None:
            ...
            print(
                f'{index} | {v_ids} |frames={len(frames)}, shape={frames[0].shape} | {labels}')
            print_line()
            print(frames[0])
            print_line()
...
def save_model_results_to_log(
    ...
    model_train_losses_log_file = os.path.join(
        model_log_dir, log_params['model_loss_info_log'])
    model_train_accuracy_log_file = os.path.join(
        model_log_dir, log_params['model_acc_info_log'])
    model_conf_mat_csv = os.path.join(
        model_log_dir, log_params['model_conf_matrix_csv'])
    model_conf_mat_png = os.path.join(
        model_log_dir, log_params['model_conf_matrix_png'])
    model_conf_mat_normalized_csv = os.path.join(
        model_log_dir, log_params['model_conf_matrix_normalized_csv'])
    model_conf_mat_normalized_png = os.path.join(
        model_log_dir, log_params['model_conf_matrix_normalized_png'])
    model_loss_png = os.path.join(model_log_dir, log_params['model_loss_png'])
    model_accuracy_png = os.path.join(
        model_log_dir, log_params['model_accuracy_png'])
    all_samples_pred_csv = os.path.join(
        model_log_dir, log_params['all_samples_pred_csv'])
    model_roc_png = os.path.join(model_log_dir, log_params['model_roc_png'])

    report = None
    if predicted is not None:
        df = pd.DataFrame([sample_names, ground_truth,
        ...
        report = metrics.classification_report(
            ground_truth, predicted, target_names=list(target_class_names))
    ...
    if model_params['batch_format'] == 'simple':
        ...
        fake_best, fraction_best, _ = grid_search_for_per_frame_model(
            per_frame_csv=all_samples_pred_csv,
            log_dir=log_dir,
        ...
        gen_report_for_per_frame_model(
            ...
            log_dir=log_dir,
            report_type=report_type,
            prob_threshold_fake=fake_best,
            prob_threshold_real=0.60,
...
def get_per_video_stat(df, vid, prob_threshold_fake, prob_threshold_real):
    ...
    fake_frames_high_prob = fake_frames_prob[np.array(
        fake_frames_prob > prob_threshold_fake)]
    ...
    real_frames_high_prob = real_frames_prob[np.array(
        real_frames_prob > prob_threshold_real)]
...
def gen_report_for_per_frame_model(
        per_frame_csv=None,
        log_dir=None,
        report_type=None,
        prob_threshold_fake=0.50,
    ...
    df['norm_probability'] = df.apply(
        lambda x: norm_probability(
            x.predictions, x.probability), axis=1)
    ...
    for v in all_videos:
        ...
        final_df = final_df.append(
            {'video': v, 'num_fake_frames': num_fake_frames,
             'num_real_frames': num_real_frames,
             'total_number_frames': total_number_frames,
    ...
    model_log_dir = os.path.join(
        log_dir, log_kind, model_params['model_name'],
        report_type, model_sub_dir)
    os.makedirs(model_log_dir, exist_ok=True)
    ...
    misc = "prob_threshold_fake = {}\nprob_threshold_real = {}\nfake_fraction = {}\n".format(
        prob_threshold_fake, prob_threshold_real, fake_fraction)
...
def get_classificiton_report_simple(
        prob_threshold_fake=None,
        prob_threshold_real=None,
        fake_fraction=None,
        all_videos=None,
        df=None):
...
def grid_search_for_per_frame_model(
        per_frame_csv=None,
        log_dir=None,
        report_type=None,
        log_kind=None,
    ...
    with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
        ...
        for prob_threshold_fake in prob_threshold_fake_range:
            for fake_fraction in fake_fraction_range:
                jobs.append(
                    pool.apply_async(
                        get_classificiton_report_simple,
                        (prob_threshold_fake,
                         prob_threshold_real,
                         fake_fraction,
                         all_videos,
    ...
    for r in results:
        ...
        grid_search_df = grid_search_df.append(
            {'prob_threshold_fake': prob_threshold_fake,
             'prob_threshold_real': prob_threshold_real,
             'fake_fraction': fake_fraction, 'accuracy': report['accuracy'],
             'report': str(report)},
            ignore_index=True)
