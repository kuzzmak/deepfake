...
def generate_graphs(losses_file, ssim_report_file, model_params):
    ...
    df_ssim = pd.DataFrame(
        ...
        columns=[
            ...
            'global_batch_num',
            'mean_ssim'])
    df_ssim = df_ssim[['global_batch_num', 'mean_ssim']
                      ].set_index('global_batch_num')
    fig_ssim = df_ssim.plot(
        figsize=(
            16,
            8),
        fontsize=16,
        alpha=0.8,
        title='SSIM score vs Global batches').get_figure()
    ...
    df_losses = pd.DataFrame(
        ...
        columns=[
            ...
            'loss_real',
            'loss_fake'])
    df_losses = df_losses.drop(
        labels=['epoch', 'local_batch_num'],
        axis=1).set_index('global_batch_num')
    fig_losses = df_losses.plot(
        figsize=(
            16,
            8),
        fontsize=16,
        alpha=0.8,
        title='All losses').get_figure()
    ...
    fig_G = df_G.plot(figsize=(16, 8), fontsize=16, alpha=0.8,
                      title='Generator GAN, pixel and SSIM loss').get_figure()
    fig_G.savefig(
        os.path.join(
            plots_path,
            'Generator_gan_pixel_ssim_losses.png'))

    df_D = df_losses[['loss_real', 'loss_fake']]
    fig_D = df_D.plot(figsize=(16, 8), fontsize=16, alpha=0.8,
                      title='Discriminator fake and real loss').get_figure()
    fig_D.savefig(
        os.path.join(
            plots_path,
            'Discriminator_fake_real_losses.png'))
...
def train_MRI_GAN_model(log_dir=None, train_resume_dir=None):
    ...
    checkpoint_best_G_path = os.path.join(
        log_dir, model_params['model_name'],
        'checkpoint_best_G.chkpt')
    checkpoint_best_D_path = os.path.join(
        log_dir, model_params['model_name'],
        'checkpoint_best_D.chkpt')
    ...
    for e in range(model_params['n_epochs']):
        ...
        # more fake. We can get more real images but that would impose a lot
        # of computation power need considering the dataset size we have.

        print(f'Creating data-loaders for epoch: {e}')
        train_dataset = MRIDataset(
            mode='train',
            transforms=data_transforms,
            frac=model_params['frac'],
        )
            ...
            if global_batches_done % model_params['sample_gen_freq'] == 0:
                try:
                    ...
                    mean_ssim = get_ssim_report(
                        global_batches_done - 1, model_params, imgs, generator,
