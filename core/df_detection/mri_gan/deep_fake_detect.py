...
from core.df_detection.mri_gan.deep_fake_detect.DeepFakeDetectModel import DeepFakeDetectModel
from core.df_detection.mri_gan.deep_fake_detect.testing import test_model
from core.df_detection.mri_gan.deep_fake_detect.training import train_model
from core.df_detection.mri_gan.utils import print_banner
...
def test_saved_model(model_path, model_kind):
    ...
    test_model(
        model,
        check_point_dict['model_params'],
        criterion,
        check_point_dict['log_dir'],
        model_kind,
    )
...
def main():
    ...
    if args.train_resume_checkpoint:
        print(
            f'Resume training from checkpoint {args.train_resume_checkpoint}'
        )
        _, model_params, _, log_dir = train_model(
            log_dir,
            train_resume_checkpoint=args.train_resume_checkpoint,
        )
        model_kind = 'highest_acc'
        model_path = os.path.join(
            log_dir,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--train_resume',
        dest='train_resume_checkpoint',
        default=False,
        help='Resume the model training')
    parser.add_argument(
        '--test_saved_model',
        dest='test_saved_model_path',
        default=False,
